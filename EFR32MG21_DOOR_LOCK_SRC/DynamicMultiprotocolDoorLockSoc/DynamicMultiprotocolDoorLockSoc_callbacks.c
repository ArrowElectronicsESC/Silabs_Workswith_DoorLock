...
#endif
#include "door-lock-srs/Global.h"
#include "door-lock-srs/Gvar.h"
...
static void readDoorLockStatus(uint8_t connection);
...
#define BLE_NOTIFY_LEN 7
#define BLE_DEFAULT_PASS 1234
...
static void doorLockWriteAndFetchInfo(uint8_t connection, uint8array *writeValue);
static void doorLockRequestWrite(uint8_t connection, uint8array *writeValue);
void notifyDoorStatus(DoorLockUserNotification_t *doorStateInfo);
static void writeOtaCtrl(uint8_t connection, uint8array *writeValue);
static void writeOtaData(uint8_t connection, uint8array *writeValue);
static void createDoorLockStatusBLEPacket(DoorLockUserNotification_t *doorStateInfo);

static uint8_t bleDoorStatusPacket[BLE_NOTIFY_LEN] = {0,BLE_DOOR_LOCK_DEFAULT_INFO,0,BLE_DOOR_LOCK_DEFAULT_INFO,0};
DoorLockUserNotification_t doorStateInfoData = {0,BLE_DOOR_LOCK_DEFAULT_INFO,0,BLE_DOOR_LOCK_DEFAULT_INFO,0};
...
static const AppCfgGattServerUserReadRequest_t appCfgGattServerUserReadRequest[] =
{
  ...
  { gattdb_door_lock_state, readDoorLockStatus },
  { 0, NULL }
};
...
static const AppCfgGattServerUserWriteRequest_t appCfgGattServerUserWriteRequest[] =
{
  ...
  { gattdb_door_lock_state, doorLockWriteAndFetchInfo },
  { gattdb_door_lock_req, doorLockRequestWrite },
  { gattdb_ota_control, writeOtaCtrl},
  { gattdb_ota_data, writeOtaData},
  { 0, NULL }
};
...
static void readDoorLockStatus(uint8_t connection)
{
  emberAfCorePrintln("lock state = %d\r\n", bleDoorStatusPacket[0]);
  emberAfCorePrintln("auth type = %d\r\n", bleDoorStatusPacket[1]);

  /* Send response to read request */
  gecko_cmd_gatt_server_send_user_read_response(connection,
		  	  	  	  	  	  	  	  	  	  	gattdb_door_lock_state,
                                                0,
                                                sizeof(bleDoorStatusPacket),
                                                bleDoorStatusPacket);
...
static void writeLightState(uint8_t connection, uint8array *writeValue)
{
  for(int i =0; i < 6; i++)
  {
	  emberAfCorePrintln("Light state write; byte %d ,value :%d\r\n",i, writeValue->data[i]);
  }
...
void ZigbeeNwFunctionality(uint8_t ucmLongPress)
{
	EmberStatus status;
	emberEventControlSetInactive(buttonEventControl);
                                                       ...
                                                       EmberAfAttributeId attributeId)
{
  ...
  if (attributeId == ZCL_ON_OFF_ATTRIBUTE_ID) {
    ...
    if (status == EMBER_ZCL_STATUS_SUCCESS) {
      if (data == 0x00) {
  		UnlockDoor(MOBILE_MODE,MOBILE_MODE,0);

        dmpUiLightOff();
        notifyLight(DMP_LIGHT_OFF);
        doorStateInfoData.lockStatus = 0;
        notifyDoorStatus(&doorStateInfoData);
      } else {
		LockDoor();

        notifyLight(DMP_LIGHT_ON);
        doorStateInfoData.lockStatus = 1;
        notifyDoorStatus(&doorStateInfoData);
        dmpUiLightOn();
      }
  ...
  switch (BGLIB_MSG_ID(evt->header)) {
    ...
    case gecko_evt_gatt_server_characteristic_status_id: {
      ...
      if (StatusEvt->status_flags == GAT_SERVER_CONFIRMATION) {
        ...
        else if (StatusEvt->characteristic == gattdb_door_lock_state) {
            emberAfCorePrintln(
              "SERVER : gattdb_door_lock_state ");
        }
        notifyDoorStatus(&doorStateInfoData);
        emberAfCorePrintln(
...
static void doorLockWriteAndFetchInfo(uint8_t connection, uint8array *writeValue)
{
  emberAfCorePrintln("door Lock Write And Fetch Info state write; %d\r\n", writeValue->data[0]);
...
static uint16_t combineTwodigit(uint8_t firstDigit, uint8_t secondDigit)
{
	uint16_t findPower = 10;
    while(secondDigit >= findPower)
    {
    	findPower *= 10;
    }
    return ((firstDigit * findPower) + (secondDigit));
}
...
static void doorLockRequestWrite(uint8_t connection, uint8array *writeValue)
{
  for(int i =0; i < 6; i++)
  {
	 emberAfCorePrintln(" door Lock RequestWrite state write;number digit:%d value:%d\r\n", i, writeValue->data[i]);
  }
  uint16_t passKey = 0;
  passKey = combineTwodigit(writeValue->data[1],writeValue->data[2]);
  bool passSuccess = false;
  emberAfCorePrintln(" door Lock passKey value:%d \r\n",passKey);

  lightDirection = DMP_UI_DIRECTION_BLUETOOTH;

  //fetch the password and pass for the authentication
	for(uint8_t ucmUser = 0 ; ucmUser < Users.usmCurrentEnrollmentNumber ; ucmUser++ )
	{
		if(Users.usmKeypadPassword[ucmUser] EQ passKey)
		{
			passSuccess = true;
  ...
  if( passSuccess == true )
  {
	doorStateInfoData.lockStatus = writeValue->data[0];
	doorStateInfoData.authType = MOBILE_MODE;
	doorStateInfoData.userId = 0;
	doorStateInfoData.userActionType = 0;

	createDoorLockStatusBLEPacket(&doorStateInfoData);
	emberAfCorePrintln(" door Lock passKey value: is correct%d \r\n",passKey);
	if(doorStateInfoData.lockStatus == DMP_DOOR_LOCK)
	{
		LockDoor();
	}
	else
  ...
  {
	  emberAfCorePrintln(" door Lock passKey value: is wrong %d \r\n",passKey);
	  gecko_cmd_gatt_server_send_user_write_response(connection,
		  	  	  	  	  	  	  	  	  	  	 gattdb_door_lock_state,
												 ES_WRITE_AUTH_FAIL);
  }
...
static void createDoorLockStatusBLEPacket(DoorLockUserNotification_t *doorStateInfo)
{
    ...
    emberAfCorePrintln("notify : door lock status = %d\r\n", doorStateInfo->lockStatus);
    emberAfCorePrintln("notify : door lock current userActionType = %d\r\n", doorStateInfo->userActionType);
    emberAfCorePrintln("notify : door lock current authType = %d\r\n", doorStateInfo->authType);

    bleDoorStatusPacket[0] = doorStateInfo->lockStatus;
    bleDoorStatusPacket[1] = doorStateInfo->userActionType;
    bleDoorStatusPacket[2] = doorStateInfo->userId;
    bleDoorStatusPacket[3] = doorStateInfo->authType;
    bleDoorStatusPacket[4] = doorStateInfo->userData[0];
    bleDoorStatusPacket[5] = doorStateInfo->userData[1];
    bleDoorStatusPacket[6] = doorStateInfo->totalEnrollUser;
...
void notifyDoorStatus(DoorLockUserNotification_t *doorStateInfo)
{
    createDoorLockStatusBLEPacket(doorStateInfo);
...
static void writeOtaCtrl(uint8_t connection, uint8array *writeValue){
	...
	int32_t btl_init_result, btl_erase_result;
	uint32_t offset = 0;
	BootloaderStorageSlot_t upgradeSlot ;
	case 0:
		btl_init_result = bootloader_init();
		emberAfCorePrintln("bootloader_init(): returns %d\r\n", btl_init_result);

		bootloader_getStorageSlotInfo(0,&upgradeSlot);
		emberAfCorePrintln("storage slot 0 is %d bytes\r\n",
				upgradeSlot.length );
		/*start with the beginning of the slot*/
		offset = upgradeSlot.address;
		while(offset < upgradeSlot.length ){
			WDOGn_Feed(WDOG0);
			emberAfCorePrintln("erase 2k at address %X\r\n", offset);
			bootloader_eraseRawStorage(offset,PAGESIZE);
			offset+=PAGESIZE;
		}
        emberAfCorePrintln("slot 0 erased\r\n");

		ota_image_position = 0;
		ota_in_progress = 1;
		emberAfCorePrintln("OTA started\r\n");
		break;
	case 3:
		ota_in_progress = 0;
		ota_image_finished = 1;
		emberAfCorePrintln("OTA done\r\n");
		break;
	}

	gecko_cmd_gatt_server_send_user_write_response(connection,gattdb_ota_control,0);
...
static void writeOtaData(uint8_t connection, uint8array *writeValue){
	...
	WDOGn_Feed(WDOG0);
	if(ota_in_progress){
		emberAfCorePrintln("writing %d bytes to bootloader storage\r\n",
				writeValue->len);
		bootloader_writeStorage(0,ota_image_position, writeValue->data, writeValue->len);
		ota_image_position+=writeValue->len;
	}
	gecko_cmd_gatt_server_send_user_write_response(connection,gattdb_ota_data,0);
}
